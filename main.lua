	...
	vape:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
		if (not teleportedServers) and (not shared.VapeIndependent) then
			...
			local teleportScript = [[
				loadstring(game:HttpGet('https://raw.githubusercontent.com/endmylifehahahahahahahahaha/AtomWareV3/'..readfile('newvape/profiles/commit.txt')..'/loader.lua', true), 'loader')()
			]]
...
do
	...
	-- Force tier 99 for local developer
	do
		local lplr = playersService.LocalPlayer
		_tierCache[lplr.UserId] = 99
	end
...
if not shared.VapeIndependent then
	...
	if isfile('newvape/games/' .. gameFileId .. '.lua') then
		...
		if not shared.VapeDeveloper then
			local suc, res = pcall(function()
				return game:HttpGet('https://raw.githubusercontent.com/endmylifehahahahahahahahaha/AtomWareV3/' .. readfile('newvape/profiles/commit.txt') .. '/games/' .. gameFileId .. '.lua', true)
			end)
